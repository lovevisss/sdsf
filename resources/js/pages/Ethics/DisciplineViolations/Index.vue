    ...
    <AppLayout>
        <div class="px-4 py-8 sm:px-6 lg:px-8">
            <div class="mx-auto max-w-7xl space-y-6">
                ...
                <div
                    v-if="flash.success"
                    class="rounded-lg border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-700 dark:border-emerald-800 dark:bg-emerald-950/30 dark:text-emerald-300"
                >
                    {{ flash.success }}
                </div>
                <div
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                    <Metric
                        label="当前人员年度记录数"
                        :value="selectedSummary?.violation_count ?? 0"
                    />
                    <Metric
                        label="当前人员年度扣分"
                        :value="selectedSummary?.total_deduction ?? 0"
                        tone="red"
                    />
                    <Metric
                        label="当前人员剩余分值"
                        :value="selectedSummary?.remaining_score ?? 20"
                        tone="green"
                    />
                </div>
                ...
                >
                    ...
                    />
                    <input
                        v-model="form.handler_department"
                        type="text"
                        placeholder="经办部门"
                        class="field"
                    />
                    <input
                        v-model="form.deduction_basis"
                        type="text"
                        placeholder="扣分依据"
                        class="field"
                    />
                ...
                >
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                <td
                                    class="table-cell text-gray-900 dark:text-white"
                                >
                                    {{ item.staff_no }}
                                </td>
                                <td
                                    class="table-cell text-gray-700 dark:text-gray-300"
                                >
                                    {{ item.staff_name }}
                                </td>
                                <td
                                ...
                                <td
                                    class="table-cell text-red-600 dark:text-red-400"
                                >
                                    {{ item.total_deduction }}
                                </td>
                                <td
                                    class="table-cell text-green-600 dark:text-green-400"
                                >
...
const Metric = defineComponent({
    ...
    setup(metricProps) {
        const color =
            metricProps.tone === 'red'
                ? 'text-red-600 dark:text-red-400'
                : metricProps.tone === 'green'
        ...
        return () =>
            h(
                ...
                [
                    ...
                    h(
                        'p',
                        { class: `mt-2 text-2xl font-semibold ${color}` },
...
const syncAttendance = (): void => {
    ...
    syncForm.post('/ethics/discipline-violations/sync-attendance', {
        preserveScroll: true,
    });
};
...
const typeLabel = (type: number): string =>
    props.violationTypeOptions[String(type)] ?? `类型${type}`;
