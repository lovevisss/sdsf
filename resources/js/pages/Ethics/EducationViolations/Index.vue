            ...
            <div class="mx-auto max-w-7xl space-y-6">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <div>
                        ...
                        <p class="mt-2 text-gray-600 dark:text-gray-400">
                            总分100分中的教育教学行为20分，按违规类型手工登记扣分并自动汇总。
                        </p>
                    </div>
                    <Link
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                    ...
                    >
                        ...
                        >
                            {{ selectedSummary?.violation_count ?? 0 }}
                        </p>
                    </div>
                    <div
                        class="rounded-lg bg-white p-5 shadow dark:bg-gray-800"
                    >
                        ...
                        >
                            {{ selectedSummary?.total_deduction ?? 0 }}
                        </p>
                    </div>
                    <div
                        class="rounded-lg bg-white p-5 shadow dark:bg-gray-800"
                    >
                        ...
                        <p
                            class="mt-2 text-2xl font-semibold text-green-600 dark:text-green-400"
                        >
                            {{
                                selectedSummary?.remaining_score ??
                                MAX_DIMENSION_SCORE
                            }}
                        </p>
                    </div>
                ...
                <p class="text-sm text-gray-600 dark:text-gray-400">
                    {{
                        selectedSummary
                            ? `当前查看：${selectedSummary.staff_no} - ${selectedSummary.staff_name}`
                            : '请先搜索并选择一个人员，再查看该人员的20分扣分情况。'
                    }}
                    <Link
                ...
                <div
                    class="rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm text-blue-700 dark:border-blue-700 dark:bg-blue-950/30 dark:text-blue-300"
                >
                    可选 Staff 人员总数：{{ props.staffOptionsCount }}
                    <span
                        v-if="props.staffOptionsError"
                        class="ml-2 text-red-600 dark:text-red-400"
                        >{{ props.staffOptionsError }}</span
                    >
                </div>

                <form
                    class="grid grid-cols-1 gap-3 rounded-lg bg-white p-5 shadow md:grid-cols-3 dark:bg-gray-800"
                ...
                >
                    ...
                    >
                        <option
                            v-for="item in props.academicYearOptions"
                            :key="item"
                            :value="item"
                        >
                            {{ item }}
                    ...
                    >
                        ...
                        <option
                            v-for="item in props.departmentOptions"
                            :key="item"
                            :value="item"
                        >
                            {{ item }}
                ...
                <form
                    class="grid grid-cols-1 gap-3 rounded-lg bg-white p-5 shadow md:grid-cols-2 lg:grid-cols-3 dark:bg-gray-800"
                ...
                >
                    ...
                    >
                        ...
                        <option
                            v-for="(label, key) in props.violationTypeOptions"
                            :key="key"
                            :value="Number(key)"
                        >
                            {{ key }}. {{ label }}
                    ...
                    <input
                        v-model="form.violation_at"
                        type="datetime-local"
                        class="w-full rounded-md border border-gray-300 px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-white"
                    />

                    <input
                        v-model="form.deduction_points"
                        type="number"
                        min="0.01"
                    ...
                    <textarea
                        ...
                        rows="2"
                        placeholder="备注"
                        class="rounded-md border border-gray-300 px-3 py-2 text-sm md:col-span-2 dark:border-gray-600 dark:bg-gray-900 dark:text-white"
                    />
                ...
                <div
                    ...
                    class="rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-700 dark:border-red-700 dark:bg-red-950/30 dark:text-red-300"
                >
                    <p v-for="(value, key) in form.errors" :key="key">
                        {{ value }}
                    </p>
                </div>
                ...
                >
                    <div
                        class="border-b border-gray-200 px-4 py-3 text-sm font-medium text-gray-700 dark:border-gray-700 dark:text-gray-300"
                    >
                        人员年度扣分汇总（每人满分20）
                    </div>
                    <table
                    ...
                    >
                        <thead class="bg-gray-50 dark:bg-gray-900">
                            <tr>
                                <th
                                    class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400"
                                >
                                    工号
                                </th>
                                <th
                        ...
                        >
                            ...
                            <tr
                                v-for="item in props.staffSummaries"
                                :key="item.staff_no"
                                class="cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700"
                                @click="pickStaffSummary(item)"
                            >
                                <td
                                ...
                                <td
                                    class="px-4 py-3 text-sm text-green-600 dark:text-green-400"
                                >
                ...
                >
                    ...
                    >
                        ...
                        >
                            ...
                            <tr
                                v-for="item in props.records.data"
                                :key="item.id"
                            >
                                <td
                                    class="px-4 py-3 text-sm text-gray-900 dark:text-white"
                                >
                                    {{ item.staff_no }} - {{ item.staff_name }}
                                </td>
                                <td
                                    class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300"
                                >
                                    {{ typeLabel(item.violation_type) }}
                                </td>
                                <td
                                    class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300"
                                >
                                    {{ formatDate(item.violation_at) }}
                                </td>
                                <td
                                    class="px-4 py-3 text-sm text-red-600 dark:text-red-400"
                                >
...
const filteredStaffOptions = computed(() => {
    ...
    return props.staffOptions.filter((item) => {
        const haystack =
            `${item.staff_no} ${item.name ?? ''} ${item.unit_name ?? ''}`.toLowerCase();
...
const submit = (): void => {
    ...
    if (!form.staff_no && staffQuery.value.trim()) {
        const exactMatch = props.staffOptions.find((item) => {
            const keyword = staffQuery.value.trim().toLowerCase();
            return (
                item.staff_no.toLowerCase() === keyword ||
    ...
    form.post('/ethics/education-violations', {
        preserveScroll: true,
        onSuccess: () => {
            staffQuery.value = '';
...
const applyStaffFilter = (): void => {
    ...
    router.get(
        '/ethics/education-violations',
        currentFilters({ staff_no: form.staff_no }),
        { preserveState: true },
    );
};

const clearStaffFilter = (): void => {
    router.get('/ethics/education-violations', currentFilters(), {
        preserveState: false,
    });
};
...
const pickStaffSummary = (item: StaffSummary): void => {
    ...
    router.get(
        '/ethics/education-violations',
        currentFilters({ staff_no: item.staff_no }),
        { preserveState: true },
    );
};
...
const resetListFilters = (): void => {
    filterAcademicYear.value =
        props.academicYearOptions[0] ?? props.academicYear;
